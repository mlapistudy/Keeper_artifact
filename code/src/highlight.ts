...
const codesmellHighlight = window.createTextEditorDecorationType({
    borderWidth: `0 0 3px 0`,
    borderStyle: `dotted`,
    borderColor: `green`
});
...
export class OutputHighlight {
    ...
    renderDisplayString(bug_type: string, description: string, fix_suggestion: string, test_input: any[], test_log_file: string, workspace: string): MarkdownString {
        ...
        if (test_log_file !== undefined) {
            res.appendMarkdown(`**Test Log**:\n\n[Click here to see the raw test log](${test_log_file})\n\n***\n\n`);
        }
    ...
    retrieveInJson(position: Position, codeFile: string): MarkdownString[] {
        ...
        this.mainJson.forEach((element: { lines_of_code: number[]; bug_type: string; description: string; fix_suggestion: string; code_file: string; test_log_file: string; test_input: any[]}) => {
            ...
            if (element.code_file === codeFile) {
                ...
                if (element.lines_of_code.includes(position.line)) {
                    ...
                    res.push(this.renderDisplayString(element.bug_type, element.description, element.fix_suggestion, element.test_input, element.test_log_file, this.workspace));
                }
            }
        });
        return res;
    ...
    public updateJsonDecorations() {
        ...
        ["Crash", "Accuracy", "Dead Code", "Code Smell"].forEach((bug_type) => {
            ...
            window.visibleTextEditors.forEach((editor) => {
                ...
                this.mainJson.forEach(function(bug_level: {bug_type: string; bugs: any[]}) {
                    let decorationList: DecorationOptions[] = [];
                    bug_level.bugs.forEach(function(element: { lines_of_code: number[]; bug_type: string; code_file: string; test_log_file: string; description: string; fix_suggestion: string; test_input: any[]}) {
                        // if (bug_level.bug_type === element.bug_type) {
                        decorationList = decorationList.concat(self.updateDecorations(element.code_file, element.lines_of_code, editor, self.renderDisplayString(bug_level.bug_type, element.description, element.fix_suggestion, element.test_input, element.test_log_file, self.workspace)));
                        // }
                    ...
                    if (bug_level.bug_type === "Code Smell") {console.log("Setting code smell underlines"); editor.setDecorations(codesmellHighlight, decorationList);};
                });
            });
        });
    ...
    public unhighlightAll() {
        window.visibleTextEditors.forEach((editor) => {
            ...
            console.log("Removing code smell underlines");
            //this.logging.appendLIne("Removing accuracy underlines");
            editor.setDecorations(codesmellHighlight, decorationList);
        });
