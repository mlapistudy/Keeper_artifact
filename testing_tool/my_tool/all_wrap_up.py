...
import static_checker as static_checker
...
def crash_bug_template():
  bug = {
        ...
        "fix_suggestion": "* It might be caused by the input size limit of ML API. Please validate the text/image/audio input of ML API before invoking it.",
        "test_input": []
    }
  return bug
...
def accuracy_bug_template():
  bug = {
        ...
        "description": "Your program suffers from accuracy problems: it fails on %2.f%% of the test inputs." % (100*(1-ACCURACY_THRESHOLD)),
        "test_input": [],
        "fix_suggestion": ""
    }
  return bug
...
def coverage_bug_template():
  bug = {
        ...
        "function_name": "",
        "func_def_line": -1,
        "description": "Your program contains dead code. It might be caused by improper branch predicates or syntax error.",
        "test_input": []
    }
  return bug

def code_smell_template():
  bug = {
        "bug_type": "Code Smell",
        "code_file": "",
        "lines_of_code": [],
        "function_name": "",
        "func_def_line": -1,
        "description": "The software has a code smell.",
        "fix_suggestion": ""
    }
  return bug
...
def get_acc_bug_fix(result_group, result_groups, constraint_file, test_file, old_overall_correct):
  suggestions = set()
  for api, keyword in result_group["ml_keywords"]:
    ...
    if api in ["label_detection", "object_localization", "classify_text"] and "[" in keyword:
      ...
      if api in ["label_detection", "object_localization"]:
        ...
        new_condition = [x.capitalize() for x in new_condition]
      ...
      suggest = "* If you use [%s] to judge these inputs, your program will behave correctly for %2.f%% of the test inputs, an improvement from %2.f%% of your original code." %(" OR ".join(new_condition), overall_correct*100, old_overall_correct*100)
      ...
      if acc > old_recall and fscore >= old_fscore:
        ...
        suggestions.add("* You could ajust the if-condition to include more keywords.")
...
def check_log(constraint_file, test_file, log_file, param_list, line_map):
  ...
  if not contain_success_execution:
    crash_bug["description"] = "Your program crashes on all test inputs. The source code probably contains syntax errors."
  ...
  for result_group in result_groups:
    ...
    if acc/total < ACCURACY_THRESHOLD:
      ...
      acc_bug["description"] = "Your program suffers from accuracy problems: on line [%s], it cannot identify [%s] in %2.f%% test inputs." % (jumps_after_fixed, print_ml_keyword(result_group["ml_keywords"]), (100*(1-acc/total)))
      ...
      fix_suggestion = None 
      fix_suggestion = get_acc_bug_fix(result_group, result_groups, constraint_file, test_file, acc/total) 
...
def static_check(input_file, constraint_file):
  bugs = []

  # async
  async_line = static_checker.misuse_async_api(input_file, constraint_file)
  for line in async_line:
    bug = code_smell_template()
    bug["lines_of_code"] = [line]
    bug["description"] = "Your program uses a blocking way to call an asynchronous ML API. It will slow down the execution."
    bug["fix_suggestion"] = "* It could be fixed by calling API in parallel (e.g., `threading` and `multiprocessing` library) or using its synchronous version."
    bugs.append(bug)
  
  # const
  const_line = static_checker.misuse_constant_input(input_file, constraint_file)
  for line in const_line:
    bug = code_smell_template()
    bug["lines_of_code"] = [line]
    bug["description"] = "Your program repeatedly make ML API calls with same input."
    bug["fix_suggestion"] = "* Cache the API result or move the function call outside the loop."
    bugs.append(bug)

  # parallel
  parallel_line = static_checker.misuse_parallel_api(input_file, constraint_file)
  for line in parallel_line:
    bug = code_smell_template()
    bug["lines_of_code"] = [line]
    bug["description"] = "Your program sequentially invokes multiple ML APIs on the same input. It will slow down the execution."
    bug["fix_suggestion"] = "* It could be fixed by using parallelism API (e.g., `annotate_image` and `annotate_text`) or calling APIs in parallel (e.g., `threading` and `multiprocessing` library)."
    bugs.append(bug)
...
def restructure_json(bugs):
  ...
  cov_bugs = {"bug_type": "Dead Code",
                "bugs": []}
  code_smell = {"bug_type": "Code Smell",
                "bugs": []}
  for bug in bugs:
    ...
    elif bug["bug_type"] == "Code Smell":
      del bug["bug_type"]
      code_smell["bugs"].append(bug)

  return [crash_bugs, acc_bugs, cov_bugs, code_smell]
...
def change_code_lines(curr_lines_of_code, curr_func_def_line, ori_func_def_line, ori_empty_lines):
    ...
    while i < len(ori_empty_lines):
        ...
        i += 1
    change_dict[curr_pos] = total_empty_lines
    logging.debug(f"change_dict computed to be {change_dict}")
...
def apply_test(file_name, function_name, output_file=None):
  if not os.path.exists(file_name):
    logging.error("[Error] " + file_name + " not found.")
    return
  ...
  change_code.change(file_name, constraint_file, function_name) 
  ...
  if not PYTHON_OHTER:
    cmd = "python3 solve_multi.py --m=25 --start=" +function_name+ " -o " +log_file+ " -t " +test_file+ " -f \"" +func_params+ "\" " + constraint_file
  else:
  ...
  if os.path.exists(f3_filename):
    logging.info(f"found function mapping at {f3_filename}, processing")
    ...
    for i in range(len(bugs)):
      ...
      if query_key in func_mapping_dict:
        logging.info(f"found {query_key} in func_mapping_dict json file, changing relevant parameters in bug output")
        ...
        if ori_file_name in empty_line_dict:
          logging.info(f"changing lines_of_code from {bugs[i]['lines_of_code']}")
          ...
          logging.info(f"to {bugs[i]['lines_of_code']}")
        ...
        logging.warning(f"query_key {query_key} does not exist in function mapping json file. This indicates that the functions processed by all_wrap_up.py were not properly processed by extract.py")
...
def from_plugin_json(json_file_path, output_path, log_file_path):
  ...
  #   logging.info(f"Found extracted_function.py file at {extracted_file_path}, deleting...")
  #   os.remove(extracted_file_path)
  # for potential_file in potential_files:
  #   potential_file_path = os.path.join(log_folder, potential_file)
  #   if Path(potential_file_path).is_file():
  #     logging.info(f"Found file to be deleted at {potential_file_path}, deleting...")
