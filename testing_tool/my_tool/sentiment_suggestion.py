...
def classifier_logic(pos, neg):
  ...
  coef = reg.coef_[0]
  if coef[1]>=0:
    classifier = "%0.3f*score + %0.3f*magnitude >= 0"%(coef[0],coef[1])
  else:
    classifier = "%0.3f*score - %0.3f*magnitude >= 0"%(coef[0],coef[1])
  # classifier = "Positive (Negative) text: score*(%0.3f) + magnitude*(%0.3f) >= (<) 0"%(coef[0],coef[1])
  # accuracy = reg.score(data, label)
  # return classifier, accuracy
  accuracy1 = reg.score(data[:len(pos)], [True]*len(pos))
  accuracy2 = reg.score(data[:-len(neg)], [False]*len(neg))
  return classifier, accuracy1, accuracy2
