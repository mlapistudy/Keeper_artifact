...
def get_varnames(code, all_scopes=True, definitions=True, references=True):
  # return get_varnames(code, all_scopes=True, definitions=True, references=True)
  return jedi.Script(code).get_names(all_scopes=all_scopes, definitions=definitions, references=definitions)
...
def inline_functions(Functions, target_function, content_line_by_line):
  ...
  while is_changed:
    ...
    for line in Functions[target_function].split('\n'):
      ...
      # print("=============="+line)
      all_names = get_varnames(line, all_scopes=True, definitions=True, references=True)
      col_fix = 0
        ...
        if name.name:
          if name.name in Functions.keys() and name.name != target_function and not name.is_definition():
            ...
            for to_inline in func_content_lines:
              ...
              inline_col_fix = 0
              modified = set()
              to_inline_names = get_varnames(to_inline, all_scopes=True, definitions=True, references=True)
...
def adhoc_fixes(content_line_by_line):
  ...
  while  line_no+1 < max_no:
    ...
    if ".error.message" in line_without_space:
      start = None
      end_pos = None
      all_names = get_varnames(line, all_scopes=True, definitions=False, references=True)
  ...
  for line in func_content:
          ...
          if find_default_value(used_ml_api, name.name) == "\"\"":
            ...
            if line[end:].strip().replace(" ","")[:2] == "].":
              ...
              if "." in line[end:]: # a[0].x.y
                ...
                tmp_names = get_varnames(line[name.column:], all_scopes=True, definitions=True, references=True) 
    ...
    if is_api_related and line.strip().startswith("for"):
      if not " in " in line:
        ...
        var_list_names = get_varnames(var_list, all_scopes=True, definitions=True, references=True)
    ...
    if " for " in line:
      ...
      if ("[" in line[:for_pos]) and ("]" in line[for_pos:]):
        ...
        tmp_names = get_varnames(sub_var, all_scopes=True, definitions=True, references=True)
        sub_var = tmp_names[0].name
        tmp_names = get_varnames(var_list, all_scopes=True, definitions=True, references=True)
        ...
        for name in get_varnames(var_list, all_scopes=True, definitions=True, references=True):
          if name.name:
            if name.name in api_related_vars:
              var_list = var_list.replace(name.name+".", name.name+"__")
              is_api_related_inside_list = True

        for name in get_varnames(result_var, all_scopes=True, definitions=True, references=True):
...
def remove_unrelated(func_content, list_vars, used_ml_api, imported_libs):
  ...
  var_to_solve = set()
  new_func_content = "\n".join(new_func)
  all_names = get_varnames(new_func_content, all_scopes=True, definitions=True, references=True)
  for name in all_names:
    if name.name:
      # ignore functions
...
def change(src_file, dest_file, target_function=None):
  logging.info("Process on "+str(src_file)+" ...")
  global If_statement_changes
