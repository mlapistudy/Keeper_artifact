...
def run_command(command, timeout=None):
  logging.info("Launch command: [%s]"%command)
  if timeout is None:
...
if __name__ == '__main__':
  # logging.info("PyExZ3 (Python Exploration with Z3)")
  ...
  logging.info ("Exploring " + app.getFile() + "." + app.getEntry())
  used_ml_api, ml_api_to_input, output_to_ml_api, If_statement_changes = get_ml_api(filename)
  logging.info(f"Testing API {used_ml_api}")
  ...
  try:
    ...
    for i in range(len(generatedInputs)):
      ...
      if not PYTHON_OHTER:
        ...
        command = "python3.8 "+ cov_test_filename +" --rss-limit-mb=20480 --loc_file="+ cov_test_filename
    ...
    #   logging.info(Solution_2_Path[i])
    ...
    # generate API inputs
    for i in range(len(generatedInputs)):
      logging.info(str(generatedInputs[i]) + "\t-->\t" + str(returnVals[i]))
      ...
      if len(used_ml_api) == 1:
        if used_ml_api[0] == "label_detection":
        ...
        elif used_ml_api[0] == "recognize" or used_ml_api[0] == "long_running_recognize":
          ...
          logging.info("API reverse not supported: " + used_ml_api[0])
          sys.exit(1)
      else:
        used_vision_api = []
        used_language_api = []
        ...
        for api in used_ml_api:
          if api in Vision_API:
            ...
            logging.info("API reverse not supported: " + api)
            sys.exit(1)
        ...
        if len(used_language_api)==0 and len(used_speech_api)==0:
          ...
          logging.info("API reverse not supported: " + str(used_ml_api))
          sys.exit(1)
      logging.info("=======================\n\n")
      all_inputs.add('|\n|'.join(files))
    ...
    if options.func_params and len(used_ml_api)>1:
      logging.info("--func-params currently is not supported for multiple API calls.")
      sys.exit(1)
        ...
        for generated_files in item.split('|\n|'):
          ...
          try:
            if len(generated_files.split('|\t|')) == 1:
              ...
              if not PYTHON_OHTER:
                command = "python3 "+ test_file +" --rss-limit-mb=20480 --loc_file="+ test_file
              else:
                command = "python3.8 "+ test_file +" --rss-limit-mb=20480 --loc_file="+ test_file
              if options.func_params:
                if item in all_inputs_list:
                  matched = 0
                  ...
                  run_command(command + command1 +" > "+cur_dir+"/tmp", timeout=2)
                ...
                run_command(command + " \"" + generated_file + "\" > "+cur_dir+"/tmp", timeout=2)
                record_coverage(cur_dir)
            else:
              logging.info("Do not support multiple files")
              sys.exit(1)
          except:
            pass
        f.write("\n=======================\n\n")
